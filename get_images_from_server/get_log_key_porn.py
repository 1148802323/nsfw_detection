...
def extract_porn_object_keys(log_directory, output_file, start_index=None, end_index=None, max_count=None):
    """
    从日志文件中提取Category为porn的ObjectKey
    ...
    object_keys = []
    count = 0
    total_detected = 0  # 总共检测到的porn图片数量
    ...
    # 正则表达式模式
    object_key_pattern = re.compile(r'ObjectKey: (\S+),')
    ...
    if max_count is not None:
        print(f"开始提取最多 {max_count} 个porn图片ObjectKey...")
        ...
        try:
            with open(log_file, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    # 查找ObjectKey行
                    if 'recive http post' in line and 'ObjectKey:' in line:
                        match = object_key_pattern.search(line)
                        if match:
                            current_object_key = match.group(1)

                    # 查找Category行
                    elif 'Category:' in line:
                        match = category_pattern.search(line)
                        if match and match.group(1) == 'porn' and current_object_key:
                            total_detected += 1
                            ...
                            if should_add:
                                object_keys.append(current_object_key)
                                print(f"找到第 {count} 个porn图片: {current_object_key}")

                            count += 1
                            current_object_key = None  # 重置当前ObjectKey
    ...
    print(f"完成! 总共检测到 {total_detected} 个porn图片")
...
if __name__ == "__main__":
    ...
    output_file = "porn_object_keys.txt"

    # 使用示例1: max_count模式（提取最多2000个）
    # extract_porn_object_keys(log_directory, output_file, max_count=2000)

    # 使用示例2: 范围模式（提取第100到第300个）
    # extract_porn_object_keys(log_directory, output_file, start_index=100, end_index=300)

    # 使用示例3: 从某个索引开始到末尾
    # extract_porn_object_keys(log_directory, output_file, start_index=500)

    # 使用示例4: 从开始到某个索引结束
    # extract_porn_object_keys(log_directory, output_file, end_index=1000)

    # 使用示例5: 提取所有
    # extract_porn_object_keys(log_directory, output_file)

    extract_porn_object_keys(log_directory, output_file, start_index=2000, end_index=2200)